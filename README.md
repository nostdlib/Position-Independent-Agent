...
- **Response**: `UINT32 status` + `UINT32 entryCount` + `UINT32 formatVersion (= 3)` + `entry[entryCount]` — compact variable-length entries (~26 bytes typical; the pre-v3 fixed 553-byte layout is history, kept parseable by the C2 for old field agents). The format word also carries the layout generation IN-BAND (`X-Api-Version` stays 1): the C2 reads the u32 at offset 8 — `3` is this layout, `0` the legacy stride.
