/**
 * @file syscall.riscv32.h
 * @brief RISC-V 32-bit Linux syscall number definitions.
 *
 * @details Defines the syscall number constants for the RISC-V 32-bit (RV32)
 * Linux kernel ABI. Like RV64, RV32 uses the generic (asm-generic) syscall
 * table with *at-style syscalls (openat, unlinkat, mkdirat) and modern
 * equivalents (clone, dup3, pipe2). On 32-bit, mmap is provided as SYS_MMAP2
 * with a page-shifted offset parameter.
 */
#pragma once
...
// Memory operations
constexpr USIZE SYS_MMAP2 = 222;  // 32-bit uses mmap2 (page-shifted offset)
...
constexpr USIZE SYS_CLONE = 220;  // riscv32 uses clone instead of fork
constexpr USIZE SYS_EXECVE = 221;
constexpr USIZE SYS_DUP3 = 24;    // riscv32 uses dup3 instead of dup2
constexpr USIZE SYS_WAIT4 = 260;   // Not implemented on riscv32 (ENOSYS)
constexpr USIZE SYS_WAITID = 95;
constexpr USIZE SYS_KILL = 129;
constexpr USIZE SYS_SETSID = 157;
constexpr USIZE SYS_PIPE2 = 59;   // riscv32 uses pipe2 instead of pipe
